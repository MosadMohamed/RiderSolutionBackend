    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Shifts Details Report</h1>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-px text-center row">
                        <form class="form row" action="{{ route('admin.report.shift.details') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            ...
                            <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                                ...
                                <tbody>
                                    ...
                                    <tr style="white-space: nowrap;">
                                        ...
                                        <td>
                                            {{ $Shift->Rider->RiderName }}_<span class="text-success">{{ $Shift->Rider->Office->OfficeName }}</span>
                                        </td>
                                        <td>
                                            <img src="{{ asset('images/companies/' . $Shift->Company->CompanyImage) }}" width="50">
                                            {{ $Shift->Company->CompanyNameEn }}
                                        </td>
                                        <td>{{ $Shift->ShiftDateStart }} {{ $Shift->ShiftTimeStart }}</td>
                                        <td>{{ $Shift->ShiftDateEnd }} {{ $Shift->ShiftTimeEnd }}</td>
                                        <td>{{ App\Helper\OfficeHelper::GetHoursBySeconds($Shift->ShiftTotalSeconds) }}</td>
                                    </tr>
                                    @endforeach
                                    <tr style="white-space: nowrap;">
                                        <td>#</td>
                                        ...
                                        <td>-</td>
                                        <td>{{ $Shifts->count() }}</td>
                                        <td>{{ App\Helper\OfficeHelper::GetHoursBySeconds($Shifts->sum('ShiftTotalSeconds')) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
